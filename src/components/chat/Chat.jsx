...
const Chat = ({ onBack, onOpenDetail, onCloseDetail }) => {
  const [chat, setChat] = useState();
  const [open, setOpen] = useState(false);
  const [text, setText] = useState("");
  const [cooldownActive, setCooldownActive] = useState(false);
  const [showDetail, setShowDetail] = useState(false); // state to track if detail is open
  ...
  const handleDetailToggle = () => {
    setShowDetail((prev) => !prev); // toggle show detail state
    if (!showDetail) {
      onOpenDetail(); // open detail if it's not open
    } else {
      onCloseDetail(); // close detail if it's open
    }
  };
  ...
  return (
    <div className="chat">
      <div className="top d-flex justify-content-between align-items-center">
        ...
        <button className="detail-button d-md-none" onClick={handleDetailToggle}>
